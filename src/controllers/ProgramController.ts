...
export default {
  fetchPrograms: asyncErrorHandler(
  ...
  fetchcomments: asyncErrorHandler(
    async (req: Request | any, res: Response) => {
      try {
  ...
  createProgram: asyncErrorHandler(
    async (req: Request | any, res: Response) => {
      ...
      const existingDepartmentId = await DepartmentService.getDepartmentById(
        department_id
      );
      if (!existingDepartmentId) {
        return res
          .status(401)
          .json({ message: "Department id does not exist" });
      }
  ...
  updateProgramStatus: asyncErrorHandler(
    async (req: Request, res: Response) => {
      try {
        await programService.updateProgramStatus(req.body);
        return res
          .status(200)
          .json({ message: "Program statuses updated successfully" });
      } catch (error) {
        console.error("Error deleting program:", error);
        return res.status(500).json({ message: error });
      }
  ...
  commentsInPrograms: asyncErrorHandler(
    async (req: Request | any, res: Response) => {
      try {
  ...
  updateCommentstatus: asyncErrorHandler(
    async (req: Request, res: Response) => {
      const commentId = req.params.id;
      try {
        ...
        res
          .status(500)
          .json({ error: "An error occurred while resolving the comment." });
      }
    }
  ),

  fetchProgramsByUser: asyncErrorHandler(
    async (req: Request | any, res: Response) => {
      try {
        const { status, name } = req.query;
        const { id } = req.user;
        const nameString = typeof name === "string" ? name : "";
        const programs = await programService.fetchProgramsByUser(
          id,
          status as ProgramStatus,
          nameString
        );
  ...
  fetchAllPrograms: asyncErrorHandler(
    async (req: Request | any, res: Response) => {
      try {
        const { id } = req.user;
        const programs = await programService.fetchAllPrograms(id);
