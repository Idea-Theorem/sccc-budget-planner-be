...
export default {
  ...
  createUser: asyncErrorHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!areValidRoleIds) {
        return res
          .status(400)
          .json({ message: "Invalid role IDs in the roles array" });
      }
      ...
      const existingRoleIds = await prisma.role
        .findMany({
          where: {
            id: {
              in: roles,
      ...
      const missingRoleIds = roles.filter(
        (roleId: string) => !existingRoleIds.includes(roleId)
      );
      if (missingRoleIds.length > 0) {
        return res
          .status(400)
          .json({ message: "Role IDs do not exist", missingRoleIds });
      }
  ...
  signin: asyncErrorHandler(async (req: Request, res: Response) => {
    try {
      ...
      const isValidPassword = await helpers.verifyPassword(
        password,
        user.password || ""
      );
